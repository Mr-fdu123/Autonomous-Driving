...
class GlobalConfig:
    ...
    def __init__(self):
        ...
        self.target_speed_max = 10.8 # Max speed the vehicle can reach
        ...
        self.vqa_gen = int(os.environ.get("VQA_GEN", 0))
        ...
        nuplan_cam_config =  {
            "x": 1.670100095641242,
            "y": 0.025874952985450953,
            "z": 1.5226234965806427,
            "roll": 0, # 91.4496305272084 - 90,
            "pitch": -0.5018736142717517,
            "yaw": 89.82261634857181 - 90,
            "width": 1920,
            "height": 1080,
            "fov_x": 63.71026476489851,
            "fov_y": 38.530528618360044,
            "fov": 63.71026476489851,
            "id": "CAM_F0"
        }
        ...
        # self.camera_pos = [0.80, 0.0, 1.60]  # x, y, z mounting position of the camera
        self.camera_pos = [nuplan_cam_config['x'], nuplan_cam_config['y'], nuplan_cam_config['z']]
        # self.camera_rot_0 = [0.0, 0.0, 0.0]  # Roll Pitch Yaw of camera 0 in degree
        self.camera_rot_0 = [nuplan_cam_config['roll'], nuplan_cam_config['pitch'], nuplan_cam_config['yaw']]
        ...
        self.camera_width = 1920  # Camera width in pixel during data collection
        self.camera_height = 1024  # Camera height in pixel during data collection
        self.camera_fov = 70

        self.camera_width = nuplan_cam_config['width']
        self.camera_height = nuplan_cam_config['height']
        self.camera_fov = nuplan_cam_config['fov']
        ...
        self.route_planner_max_distance = 25.0 # align to think2drive
        ...
        # Collect param
        # -----------------------------------------------------------------------------
        self.do_fail = int(os.environ.get('DO_FAIL', 0))
        self.do_recover = int(os.environ.get('RECOVER', 0))
        self.do_ood = int(os.environ.get('DO_OOD', 0))

        self.do_acc = int(os.environ.get('DO_ACC', 0))
        self.do_dec = int(os.environ.get('DO_DEC', 0))
        
        self.entry_point = float(os.environ.get('P_START', 1.0)) # start
        self.current_point = float(os.environ.get('P_MOVE', 2.0)) # special move
        # self.hold_point = float(os.environ.get('P_HOLD', 3.5)) # keep straight, now use delta angle
        self.end_point = float(os.environ.get('P_END', 7.0)) # end
        self.exit_point = float(os.environ.get('P_EXIT', 10.0)) # exit
        self.wp_save_freq = 10
        self.MARGIN = 2.0
        self.out_filename = "waypoints"
        self.direction_flipper = True

        # -----------------------------------------------------------------------------
        # VQA generator configs
        # -----------------------------------------------------------------------------
        self.work_dir = os.environ.get('WORK_DIR', '.')
        self.model_name = os.environ.get('EXPERT_NAME', 'default_expert')
        
        self.folder_str = f"{self.model_name}"
        ...
        self.vqa_args = VQAArgs(self.output_example_dir, self.output_graph_dir)

        self.vlm_freq = 5

class VQAArgs():
    def __init__(self, output_example_dir, output_graph_dir):
        import os
