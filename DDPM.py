...
class Diffusion:
    ...
    def p_sample(self, x_t, t, model):
        # x_t -> x_t-1
        sqrt_recip_alphas_t = self.extract(self.sqrt_recip_alphas, t)
        betas_t = self.extract(self.betas, t)
        sqrt_one_minus_alphas_cumprod_t = self.extract(self.sqrt_one_minus_alphas_cumprod, t)
        posterior_variance_t = self.extract(self.posterior_variance, t)

        eps_pred = model(x_t, t)
        mean_pred = sqrt_recip_alphas_t * (x_t - betas_t / sqrt_one_minus_alphas_cumprod_t * eps_pred)
        eps = torch.randn_like(x_t)
        x_pred = mean_pred + torch.sqrt(posterior_variance_t) * eps
        return x_pred
    
    @torch.no_grad()
    def sample(self, model, height, width, batch_size, channels):
